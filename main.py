...
"Project LEGO" is the second version of ccbuild, superceding it.

The build configuration is written in JSON, and unlike GYP, it actually compiles
and builds source and dependencies.

The <devroot> is the base from where the build starts. The search order is:
commandline flag (--devroot), config file ($HOME/.lego.json) or a backtrace
for a file, ".legocfg.json", in the parent directories of the current directory,
stopping at $HOME or /usr/local. It expects the usual directories, "src",
"include", "lib", "build" and "bin" to exist there, though it may not use all
of them. Only the "src" directory is required; "build" is created if it
doesn't exist. "src" becomes the source base.

Build configurations can be specified in $HOME/.legocfg.json. It is recommended
that this file includes all non-standard flags, include paths and library paths
applicable to all builds.

EXAMPLE:
  $ lego build //cc/net:socket             # :socket is a library target
  Source: <devroot>/src/cc/net/{build.json, socket.h, socket.cc}
  Build: <devroot>/build/cc/net/socket.dylib

  $ lego run //cc/net:socket_try           # :socket_try is a binary target
...
Using the name "LEGO": I like LEGO, so have named this tool after it, in spirit
of building things.
"""
...
def main(argv):
  ...
  # Configure build values based on $HOME/.legocfg.json
  # Override build values based on the first .legocfg.json in parent dirs.
  # Build!
  return NO_ERROR
